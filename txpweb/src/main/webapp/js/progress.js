$(function () {
    ...
    if (isExistLogin()) {
        ...
        $.ajax({// 最近一期工单查询 (过滤掉了1 草稿状态工单)
            type: "post",
            url: "/loanapp-api-web/bewadwallet/loan/findBwOrderByBwId.do",
            dataType: "json",
            data: {
                "bwId": bwId,
            ...
            success: function (data) {
                if (data.result != null) {// ****************************有工单信息
                    ...
                    if (statusId == '4') {// 签约
					   ...
					   if(repayType==null){
                    	...
                    	// ////////////
                         $.ajax({
                             type: "post",
                             url: "/loanapp-api-web/app/my/appCheckLogin/checkBankCardSignByBwId.do",
                             dataType: "json",
                             data: {
                                 "loginToken": loginToken,
                                 "bwId": bwId
                    ...
                    if (statusId == '9') {// 还款中
					   window.location.href = "/loanapp-api-web/bewadwallet/loan/repayNormal.do?orderId="+orderId;
                    }

                    if (statusId == '13') {// 逾期
					  window.location.href = "/loanapp-api-web/bewadwallet/loan/repayLate.do?orderId="+orderId;
                    }
                    // ////////////////////////////
                } else {// 无工单信息
                    window.location.href = "/loanapp-api-web/bewadwallet/loan/haoDaiController.do";
                }
...
function sign() {
	 if(isExistLogin()){//登录成功
       ...
       //  }
		...
		$.ajax({//修改还款方式和借款时间
			type: "post",
			url: "/loanapp-api-web/app/my/appCheckLogin/updateBwOrderAttrByOrderId.do",
			dataType: "json",
			data: {
				"orderId":orderId,
			...
			success: function (data) {
				...
				////////////////////////////////////////////
					...
					// ////////////借款人基本信息
						$.ajax({
							type: "post",
							url: "/loanapp-api-web/app/my/appCheckLogin/findBwBorrowerByBwId.do",
							dataType: "json",
							data: {
								"loginToken": getLoginToken(),
								"bwId": getUUID()
						...
						alert("签约成功");
						location.href="/loanapp-api-web/bewadwallet/loan/haoDaiController.do"
					}
					////////////////////////////////////////////
				}else{
					alert("系统异常")

$(function () {
    ...
    // 判断请求是否是从公众号跳转过来
    if (code != null && state == "loanapp-api-web" && isWeixin()) {
        getOpenIdIndex(code);
    }
	// 判断是否为微信浏览器
	if (isWeixin()) {
...
function getOpenIdIndex(code) {
	$.ajax({
        url: "/loanapp-api-web/weixin/getOpenIdIndex.do",
        type: "post",
        dataType: "json",
        async: false,
        data: {"code" : code},
        success: function () {
            $.ajax({url: "/loanapp-api-web/app/borrower/weiXinIndex.do", type: "post", async: false});
        }
    });
	/*$.post("/loanapp-api-web/weixin/getOpenIdIndex.do", {"code": code}, function(res){
		if (res.code == "000") {
			$.post("/loanapp-api-web/app/borrower/weiXinIndex.do");
		}
	});*/
}
	...
	$.post(
		...
		function(data) {
			...
			wx.ready(function(){
				...
				var state = getParameter("state");
				// 判断请求是否是从公众号跳转过来
				if (code != null && state == "loanapp-api-web") {
							...
							var geocoder = new qq.maps.Geocoder(
									{
										complete: function (
											...
											$.post(
															"/loanapp-api-web/weixin/getLocationIndex.do",
															{
																"location": location
															},
															function (
...
function getChannel() {
	...
	if (cid != null) {
		$.post("/loanapp-api-web/app/borrower/getChannel.do", {channelCode : cid});
	}
}
...
function getLocation() {
	new qq.maps.Geolocation("QGUBZ-QLYKK-H6YJQ-A4ZK7-R2OC7-6EFUT", "H5").getLocation(function(pos) {
		...
		$.post("/loanapp-api-web/weixin/getLocationIndex.do", {"location": location}, function(data) {
			if (data.code != "000") {
				//alert("定位失败");
			}
		},"json");
...
function borrowMoney() {
    ...
    if (isExistLogin()) {
    	...
    	new qq.maps.Geolocation("QGUBZ-QLYKK-H6YJQ-A4ZK7-R2OC7-6EFUT", "H5").getLocation(function(pos) {
    		 $.ajax({
                 type: "post",
                 url: "http://139.224.17.43/loanapp-api-web/app/cityCheck/appCheckLogin/cityCheck.do",
                 dataType: "json",
                 data: {
                     "loginToken": loginToken,
                     "cityName": pos.city
                 ...
                 success: function (data) {
                     if (data.code!= "000") {//不支持借款
                        alert(data.msg);
                         ...
                         $.ajax({
                             type: "post",
                             url: "http://139.224.17.43/loanapp-api-web/app/my/appCheckLogin/generateBwOrderForAuthThird.do",
                             dataType: "json",
                             data: {
                                 "loginToken": loginToken,
                                 "bwId": bwId,
                             ...
                             success: function (data) {
                                 if (data.code == "000") {
                 					...
                 					$.post("/loanapp-api-web/app/my/appCheckLogin/findBwBorrowerByBwId.do",
                                         {
                                             "loginToken": loginToken,
                                             "bwId": bwId
                                         }, function (data) {

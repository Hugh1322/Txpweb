...
public class RedisUtils {
	...
	static {
		...
		// config.setMaxActive(SystemConstant.REDIS_MAXACTIVE);
		// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
		config.setMaxIdle(SystemConstant.REDIS_MAXIDLE);
		// 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
		// config.setMaxWait(SystemConstant.REDIS_MAXWAIT);
		...
		// pool = new JedisPool(config, SystemConstant.REDIS_IP, SystemConstant.REDIS_PORT,
		// SystemConstant.REDIS_TIMEOUT);
//		pool = new JedisPool(config, SystemConstant.REDIS_IP, SystemConstant.REDIS_PORT, SystemConstant.REDIS_TIMEOUT,
//				SystemConstant.REDIS_PASSWORD);
//		pool = new JedisPool(config, "127.0.0.1", 6379, SystemConstant.REDIS_TIMEOUT,
//				null);
		pool = new JedisPool(config, "106.14.238.126", 6379, SystemConstant.REDIS_TIMEOUT,
				"beadwalletredis",14);
	}
